...
if __name__ == '__main__':
    ...
    options = {
        # 'weeks_to_import': [1, 2, 4, 5, 6, 8, 9, 10, 11, 12, 13, 14, 16, 17],
        'weeks_to_import': [3, 7, 15],
    ...
    # Process weeks one at a time to minimise memory overhead
    for week in total_weeks:
        ...
        # Process selected games/plays using the 'Ray' multiprocessing framework, each process handles one play at a time
        ...
        # Clear non-essential data from memory to conserve resources
        del results

        # Set up for export of data
        result_files = [
            (tracking_results, 'tracking_results.csv',
             ['time', 's', 'a', 'dis', 'event', 'nflId', 'displayName', 'jerseyNumber', 'position', 'frameId', 'team',
              'gameId', 'playId', 'playDirection', 'route', 'pos', 'teamType', 'o_vec', 'dir_vec', 'r_vec']),
            (frame_results, 'frame_results.csv',
             ['gameId', 'playId', 'frameId', 'offense_p_group', 'defense_p_group', 'offense_m_group', 'defense_m_group',
              'o_state', 'd_state', 'offense_v_group', 'defense_v_group', 'offense_a_group', 'defense_a_group',
              'a_group_ratio']),
            (play_results, 'play_results.csv',
             ['gameId', 'playId',
              'offense_h_play', 'offense_h_presnap', 'offense_h_to_throw', 'offense_h_to_arrived',
